...
pub mod pallet {
    ...
    pub enum Event<T: Config> {
        ...
        /// Event emitted when a proof has been transferred. [who, claim, target]
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub(super) fn transfer_claim(
            ...
            // Store the proof with the target and block number.
            Proofs::<T>::insert(&proof, (&target, current_block));

            // Emit an event that the claim was created.
            Self::deposit_event(Event::ClaimTransferred(sender, proof, target));
